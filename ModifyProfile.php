...
function assign_token(){
    $token_cookie = setcookie("csrfTkn", base64_encode(openssl_random_pseudo_bytes(32)), 0, "/");
    
    return  $token_cookie;
}
function validateToken( $token)
{
    return urldecode($token) ==$_COOKIE['csrfTkn'];
}
assign_token();
  ...
  <body>
        <div class="login-form">
            <form id="modifyDetails" class="form-horizontal" role= "form" action="ModifyProfile.php" method="post">
                ...
                <div class="form-group">
                    <input id="ssd-csrf" type="hidden" class="form-control" name="csrf_token_2" value=<?php echo '"' . assign_token() . '"';?>>
                </div>
...
document.getElementById('ssd-csrf').value=getCookie('csrfTkn');
</script>